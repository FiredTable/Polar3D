...
from core_stereo_rt.utils.augmentor import FlowAugmentor, SparseFlowAugmentor
    ...
    def __init__(self, aug_params=None, root='../datasets/middlebury', split='2014', resolution='F'):
        ...
        if split == "2005":
            ...
            for scene in scenes:
                ...
                for l in ["L0"]:
                    for s in ["0", "1", "2", "3"]:
                        name_number = scene.name 
                        name = name_number[:-1]
                        file0 = os.path.join(scene.parent.parent, f'ambient-{name}', 'data', f'{name_number}', f'ambient/{l}/im0e{s}.png')
                        if os.path.exists(file0):
                            file1 = os.path.join(scene.parent.parent, f'ambient-{name}', 'data', f'{name_number}', f'ambient/{l}/im1e{s}.png')
                            disp_file = str(scene / "disp0.pfm")

                            self.image_list += [[file0, file1]]
