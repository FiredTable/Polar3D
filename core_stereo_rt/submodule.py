...
class GWCorrelation(nn.Module):
    ...
    def forward(self, fea1, fea2):
        ...
        cost = (fea1 * fea2).view([B, self.num_groups, channels_per_group, H, W]).mean(dim=2)
        assert cost.shape == (B, self.num_groups, H, W)
        return cost


class ContextAwareGWCorrelation(nn.Module):
    def __init__(self, in_channels, num_groups, reduction_ratio=8):
        super().__init__()
        self.num_groups = num_groups
        self.channels_per_group = in_channels // num_groups

        self.context_net = nn.Sequential(
            nn.Conv2d(2 * in_channels, in_channels // reduction_ratio, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels // reduction_ratio, in_channels, 3, padding=1),
            nn.Sigmoid()
        )

        self.group_conv = nn.Conv2d(in_channels, num_groups, 1, bias=False)
    
    def forward(self, fea1, fea2):
        B, C, H, W = fea1.shape
        assert C % self.num_groups == 0

        pointwise_corr = fea1 * fea2
        context_input = torch.cat([fea1, fea2], dim=1)
        attention = self.context_net(context_input)  # [B, C, H, W]

        enhanced_corr = pointwise_corr * attention
        group_corr = self.group_conv(enhanced_corr)  # [B, num_groups, H, W]
...
def context_upsample(disp_low, up_weights):
    ...
    # sp (b,9,4*h,4*w)
    ###
    b, c, h, w = disp_low.shape  # [B, 1, H/4, W/4]    
    disp_unfold = F.unfold(disp_low.reshape(b,c,h,w),3,1,1).reshape(b,-1,h,w)  # [B, 9, h, w]
    disp_unfold = F.interpolate(disp_unfold,(h*4,w*4),mode='nearest').reshape(b,9,h*4,w*4) # [B, 9, H, W]
