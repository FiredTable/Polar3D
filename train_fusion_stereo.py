...
from utils import tensor_imsave
from collections import OrderedDict
from core_fusion_stereo.rt_igev_stereo import PFIGEVStereo
from core_fusion.loss import MyLoss as FusionLoss
from evaluate_fusion_stereo import *
...
def sequence_loss(agg_pred, iter_preds, disp_gt, valid, 
                  left_img1, left_img2, fused_left, 
                  right_img1, right_img2, fused_right, loss_gamma=0.9):
...
def train(args):
    model = nn.DataParallel(PFIGEVStereo(args))
    ...
    if args.restore_ckpt is not None:
        ...
        if args.restore_ckpt_fusion is not None:
            assert args.restore_ckpt_fusion.endswith(".pth")
            logging.info(f"Loading checkpoint {args.restore_ckpt_fusion}")
            checkpoint = torch.load(args.restore_ckpt_fusion)
            model.module.fusion_model.load_state_dict(checkpoint, strict=True)
            logging.info(f"Done loading checkpoint")

        if args.restore_ckpt_stereo is not None:
            assert args.restore_ckpt_stereo.endswith(".pth")
            logging.info(f"Loading checkpoint {args.restore_ckpt_stereo}")
    ...
    model.cuda()
    model.train()
    model.module.stereo_model.freeze_bn() # We keep BatchNorm frozen
    ...
    while should_keep_training:
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            left_img1, right_img1, left_img2, right_img2, disp_gt, valid = [x.cuda() for x in data_blob]

            assert model.training
            agg_pred, iter_preds, fused_left, fused_right = model(left_img1, right_img1, left_img2, right_img2, iters=args.train_iters)
            assert model.training

            loss, metrics = sequence_loss(agg_pred, iter_preds, disp_gt, valid, 
                                          left_img1, left_img2, fused_left, 
                                          right_img1, right_img2, fused_right)
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                ...
                model.train()
                model.module.stereo_model.freeze_bn()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='pf-igev-stereo', help="name your experiment")
    parser.add_argument('--restore_ckpt', default=None)
    parser.add_argument('--restore_ckpt_fusion', default=None)
    parser.add_argument('--restore_ckpt_stereo', default='./checkpoints_stereo/vits_cagwc/2025-09-26_22-18_sceneflow/ca-igev-stereo.pth')
    parser.add_argument('--logdir', default='./checkpoints_fusion_stereo', help='the directory to save logs and checkpoints')
    ...
    parser.add_argument('--image_size', type=int, nargs='+', default=[128, 384], help="size of the random image crops used during training.")
    ...
    args.logdir = os.path.join(args.logdir, args.encoder + '_' + args.corr_module, time_str+f'_{args.train_datasets}')
