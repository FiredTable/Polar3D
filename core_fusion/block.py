...
__all__ = [
    ...
    'NestDecoder', 'FSDecoder', 'Downsample', 'Upsample', 'SAIB', 'SACB'
]
...
class DenseBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, num_convs=3):
        ...
        self.layers = nn.ModuleList(
            [ConvLayer(in_ch + i * out_ch, out_ch, act=nn.LeakyReLU) for i in range(num_convs)])
...
class SAIB(nn.Module):
    """Spatial Attention-guided Inter-domain Fusion Block"""
    def __init__(self, ksize=16, num_ch=64):
        super().__init__()
        self.ksize = ksize
        self.down = nn.AvgPool2d(ksize, ksize)
        self.up = nn.Upsample(scale_factor=ksize, mode='bilinear', align_corners=False)
        self.norm1 = nn.InstanceNorm2d(num_ch)

        self.ffn = nn.Sequential(
            nn.Conv2d(num_ch, num_ch, 3, padding=1, groups=num_ch),
            nn.ReLU6(inplace=True)
        )
        self.norm2 = nn.InstanceNorm2d(num_ch)

    def forward(self, x):
        ksize = self.ksize
        b, c, h, w = x.shape
        assert h % ksize == 0 and w % ksize == 0

        down_x = self.down(x)
        b, c, h_prime, w_prime = down_x.shape
        n_prime = h_prime * w_prime

        down_flat = down_x.view(b, c, n_prime)

        if False:
            q = down_flat.permute(0, 2, 1)  # [B,(HW//64),C]
            k = down_flat  # [B,C,(HW//64)]
            energy = q @ k / (c ** 0.5) # [B,(HW//64),(HW//64)]
            energy = energy.softmax(dim=-1)

            v = down_flat.permute(0, 2, 1)  # [B,(HW//64),C]
            attn = energy @ v  # [B,(HW//64),C]
        else:
            attn = torch.zeros(b, n_prime, c, device=x.device, dtype=x.dtype)
            chunk_size = 8192  # 根据GPU内存调整分块大小
            for i in range(0, n_prime, chunk_size):
                end_i = min(i + chunk_size, n_prime)
                q_chunk = down_flat.permute(0, 2, 1)[:, i:end_i]  # [B, chunk, C]
                for j in range(0, n_prime, chunk_size):
                    end_j = min(j + chunk_size, n_prime)
                    k_chunk = down_flat[:, :, j:end_j]  # [B, C, chunk_j]
                    v_chunk = down_flat.permute(0, 2, 1)[:, j:end_j]  # [B, chunk_j, C]

                    # 计算分块能量
                    energy_chunk = torch.einsum('bic,bcj->bij', q_chunk, k_chunk) / (c ** 0.5)
                    energy_chunk = energy_chunk.softmax(dim=-1)

                    # 累积注意力
                    attn_chunk = torch.einsum('bij,bjc->bic', energy_chunk, v_chunk)
                    attn[:, i:end_i] += attn_chunk
        
        attn = attn.permute(0, 2, 1).reshape(b, c, h // ksize, w // ksize)  # [B,C,H//8,W//8]
        attn = self.up(self.norm1(attn))  # [B,C,H,W]
...
class SACB(nn.Module):
    """Spatial Attention-guided Cross-domain Fusion Block"""
    def __init__(self, ksize=16, num_ch=16):
        super().__init__()
        self.ksize = ksize
        self.down = nn.Conv2d(num_ch, num_ch, kernel_size=ksize, stride=ksize)
        self.up = nn.Upsample(scale_factor=ksize, mode='bilinear', align_corners=False)
        self.norm1 = nn.InstanceNorm2d(num_ch)
    ...
    def cross_attn(self, query, key_value):
        """计算跨模态注意力"""
        if False:
            b, c, h, w = query.shape
            q = self.down(query).reshape(b, c, -1).permute(0, 2, 1)  # [B, N, C]
            kv = self.down(key_value).reshape(b, c, -1)  # [B, C, N]

            energy = q @ kv / (c ** 0.5)  # [B, N, N]
            attn_map = energy.softmax(dim=-1)  # 按关键特征归一化

            v = kv.permute(0, 2, 1)  # [B, M, C]
            attn = attn_map @ v  # [B, N, C]
            attn = attn.permute(0, 2, 1).reshape(b, c, h//self.ksize, w//self.ksize)
            return self.up(self.norm1(attn))  # 上采样到原始尺寸
        else: 
            b, c, h, w = query.shape
            # 下采样并展平
            down_query = self.down(query)  # [B, C, H', W']
            down_kv = self.down(key_value)  # [B, C, H', W']
            
            b, c, h_prime, w_prime = down_query.shape
            n_prime = h_prime * w_prime
            
            # 展平为序列
            q = down_query.view(b, c, n_prime).permute(0, 2, 1)  # [B, N, C]
            k = down_kv.view(b, c, n_prime)  # [B, C, N]
            v = down_kv.view(b, c, n_prime).permute(0, 2, 1)  # [B, N, C]
            
            # 初始化注意力输出
            attn = torch.zeros(b, n_prime, c, device=query.device, dtype=query.dtype)
            
            # 分块计算
            chunk_size = 8192  # 根据GPU内存调整
            for i in range(0, n_prime, chunk_size):
                end_i = min(i + chunk_size, n_prime)
                q_chunk = q[:, i:end_i]  # [B, chunk, C]
                
                # 计算分块能量
                energy_chunk = torch.zeros(b, end_i-i, n_prime, device=q.device, dtype=q.dtype)
                
                # 分块计算矩阵乘法
                for j in range(0, n_prime, chunk_size):
                    end_j = min(j + chunk_size, n_prime)
                    k_chunk = k[:, :, j:end_j]  # [B, C, chunk_j]
                    
                    # 计算部分能量
                    energy_part = torch.einsum('bic,bcj->bij', q_chunk, k_chunk) / (c ** 0.5)
                    energy_chunk[:, :, j:end_j] = energy_part
                
                # 应用softmax
                energy_chunk = energy_chunk.softmax(dim=-1)
                
                # 分块计算注意力值
                attn_chunk = torch.zeros(b, end_i-i, c, device=q.device, dtype=q.dtype)
                for j in range(0, n_prime, chunk_size):
                    end_j = min(j + chunk_size, n_prime)
                    v_chunk = v[:, j:end_j]  # [B, chunk_j, C]
                    energy_part = energy_chunk[:, :, j:end_j]
                    
                    # 计算部分注意力
                    attn_part = torch.einsum('bij,bjc->bic', energy_part, v_chunk)
                    attn_chunk += attn_part
                
                attn[:, i:end_i] = attn_chunk
            
            # 恢复形状并上采样
            attn = attn.permute(0, 2, 1).view(b, c, h_prime, w_prime)
            return self.up(self.norm1(attn))

    def forward(self, x1, x2):
        # 双向注意力
        attn1_to_2 = self.cross_attn(x1, x2)  # x1关注x2
        attn2_to_1 = self.cross_attn(x2, x1)  # x2关注x1

        # 门控融合
        gate1 = self.gate(torch.cat([x1, attn1_to_2], dim=1))
        gate2 = self.gate(torch.cat([x2, attn2_to_1], dim=1))

        x1_fused = x1 * (1 - gate1) + attn1_to_2 * gate1
        x2_fused = x2 * (1 - gate2) + attn2_to_1 * gate2

        # 特征增强
        return self.ffn(x1_fused), self.ffn(x2_fused)
