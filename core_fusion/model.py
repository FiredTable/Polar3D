...
__all__ = [
    ...
    'IFCNN', 'DIFNet', 'PMGI', 'MyFusion', 'APFNet'
]
...
class APFNet(nn.Module):
    def __init__(self):
        ...
        self.spatial_attention_block1 = SAIB(ksize=8, num_ch=64)
        self.spatial_attention_block2 = SAIB(ksize=8, num_ch=64)
        self.cross_spatial_attention_block = SACB(ksize=8, num_ch=64)
        ...
        self.decode = nn.Sequential(
            ...
            ConvLayer(64, 32),
            ConvLayer(32, 16),
            ConvLayer(16, 1, act=nn.Tanh),
        )

    def encoder(self, img1, img2):
        feat1 = self.encode1(img1)  # [B, 64, H, W]
        feat2 = self.encode2(img2)  # [B, 64, H, W]

        return feat1, feat2

    def fusion(self, feat1, feat2):
        spatial1 = self.spatial_attention_block1(feat1)
        spatial2 = self.spatial_attention_block2(feat2)

        fused1, fused2 = self.cross_spatial_attention_block(spatial1, spatial2)

        f_fused = concat_fusion((fused1, fused2), dim=1)  # [B, 128, H, W]
        return f_fused  
    ...
    def forward(self, img1, img2):
        # extract
        feat1, feat2 = self.encoder(img1, img2)
