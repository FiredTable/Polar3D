...
def spatial_fusion(tensor1, tensor2, mode='l1', softmax=True):
    spatial1 = spatial_pooling(tensor1, mode, ksize=8)
    spatial2 = spatial_pooling(tensor2, mode, ksize=8)
...
def spatial_pooling(tensor, mode='l1', ksize=8):
    if mode == 'sum':
        ...
        q = F.avg_pool2d(tensor, ksize, ksize).reshape(b, c, -1).permute(0, 2, 1)  # [B,(HW//64),C]
        k = F.avg_pool2d(tensor, ksize, ksize).reshape(b, c, -1)  # [B,C,(HW//64)]
        energy = q @ k / (c ** 0.5) # [B,(HW//64),(HW//64)]
        energy_softmax = F.softmax(energy, dim=-1)

        v = F.avg_pool2d(tensor, ksize, ksize).reshape(b, c, -1).permute(0, 2, 1)  # [B,(HW//64),C]
        attn = energy_softmax @ v  # [B,(HW//64),C]
        attn = attn.permute(0, 2, 1).reshape(b, c, h // ksize, w // ksize)
...
def channel_pooling(tensor, mode='avg', ksize=8):
    b, c, h, w = tensor.shape
    ...
    if mode == 'avg':
        ...
        q = F.avg_pool2d(tensor, ksize, ksize).reshape(b, c, -1)  #[B,C,(HW//64)]
        k = F.avg_pool2d(tensor, ksize, ksize).reshape(b, c, -1).permute(0, 2, 1)  #[B,(HW//64),C]
        energy = q @ k / (c ** 0.5) #[B,C,C]
        energy_softmax = F.softmax(energy, dim=-1)

        v = F.avg_pool2d(tensor, ksize, ksize).reshape(b, c, -1)  #[B,C,(HW//64)]
        attn = energy_softmax @ v  #[B,C,(HW//64)]
        attn = attn.reshape(b, c, h // ksize, w // ksize)
        attn = F.interpolate(attn, size=(h, w), mode='bilinear', align_corners=False)
