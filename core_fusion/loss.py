...
class MyLoss(nn.Module):
    def __init__(self, lambda1=1.0, lambda2=0.1, lambda3=10.0):
        ...
        self.win_sizes = win_sizes = (11, 9, 7, 5, 3)
        data_range = 1.0
        use_padding = True
        size_average = False
        self.ssim_fns = [SSIM(win_size, data_range, use_padding, size_average)
                         for win_size in win_sizes]
        self.grad_loss = GradLoss('l1', lambda2)
    ...
    def forward(self, img1, img2, imgf):
        ...
        msw_ssim = ssim / len(self.win_sizes)
        msw_ssim_loss = self.lambda1 * (1.0 - msw_ssim)

        intensity_loss = self.lambda3 * F.mse_loss(imgf, torch.max(img1, img2))
        grad_loss = self.grad_loss(img1, img2, imgf, mode='max')

        return msw_ssim_loss, grad_loss, intensity_loss
